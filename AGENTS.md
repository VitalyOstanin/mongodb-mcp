...
- Maintain `README.md` in English and `README-ru.md` in Russian so both stay aligned with the MongoDB MCP feature set.
...
## Project Structure
- `src/`: TypeScript sources for the MCP server, MongoDB client, and tool registrations.
...
- Keep documentation (`README*`, `TODO*`, ru variants when available) aligned with the current MongoDB feature set after each iteration.
...
- To run the built server locally, execute `MONGODB_URL="mongodb://localhost:27017" node dist/index.js` (replace with real credentials).
- For rapid manual testing without a full client, you can pipe JSON-RPC messages directly:
  ...
  2. In another terminal, send the initialization sequence by writing JSON-RPC lines directly to the server STDIN:
     ```bash
     cat <<'JSON' | MONGODB_URL=... node dist/index.js
...
export function registerFindTool(server: McpServer, client: MongoClient) {
  server.registerTool(
    "find",
    {
      description: "Find documents in a MongoDB collection. Use for: Querying data from MongoDB collections.",
...
registerFindTool(this.server, this.client);
```
