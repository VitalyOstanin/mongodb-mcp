...
- **Защита только для чтения**: Когда используется флаг `--read-only` (по умолчанию), операции записи заблокированы. Используйте `--read-only=false`, чтобы разрешить операции записи.
